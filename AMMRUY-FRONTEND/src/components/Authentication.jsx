...
function Authentication() {
  ...
  return <main className='md:flex md:justify-between md:items-center md:gap-24 max-h-full'>
    <div className="signup text-center w-[100vw] mx-auto">
      <div className="content md:ml-72 md:w-[60vmin] mx-auto w-[80%]">
        ...
        <div className="already md:hidden flex gap-1">
          <p>{!login ? 'Already have an account ?' : 'dont have a account ?'}</p>
          <span
            onClick={loginHandler}
            className='text-blue-600'>
            {!login ? 'Login' : 'signup'}</span>
        </div>
      </div>
    </div>
